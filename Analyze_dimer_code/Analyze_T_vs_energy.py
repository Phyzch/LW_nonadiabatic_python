...
def analyze_T_T_prime_vs_energy_main():
    ...
    # Bigwood formula for scaling factor,  T + T'
    # plot_T_T_prime_estimate_for_data_vs_energy_V0_3050()

    # Bigwood formula for scaling factor, T + T'. energy in one monomer
    plot_T_T_prime_estimate_for_data_vs_energy_V0_3050_energy_in_single_monomer()
...
def plot_T_T_prime_estimate_for_data_vs_energy_V0_3050_energy_in_single_monomer():
    '''
    ...
    fig_path = "/home/phyzch/Presentation/LW_electronic_model/2022 result/spin_boson_LW/BChl_dimer_model/5_mode/" \
               "batch_simulation_Bigwood_scaling/batch_simulation_energy_in_one_monomer/"
    Vt = 363
    # estimate using equal energy per monomer assumption
    alpha_list = np.array([0.169, 0.163, 0.127, 0.101, 0.101])
    ...
    # estimate energy and T+T' for sampled states.
    file_path = "/home/phyzch/Presentation/LW_electronic_model/2022 result/spin_boson_LW/BChl_dimer_model/" \
                "5_mode/batch_simulation_Bigwood_scaling/batch_simulation_energy_in_one_monomer/output_file/Vt=363/"

    # T_dimer = T_monomer1 + T_monomer2
    dimer_alpha = [alpha_list  , alpha_list]
    dimer_frequency = [ frequency_list , frequency_list ]
    T_states = estimate_transition_factor_T_dimer_lists(V0, scaling_factor, file_path)
    T_prime_states = estimate_nonadiabatic_transition_factor_T_prime_dimer_list(dimer_alpha, dimer_frequency,
                                                                                file_path, Vt)
    ...
    T_list_energy_in_one_monomer = []
    T_prime_list_energy_in_one_monomer = []

    for i in range(energy_data_num):
        energy = energy_list[i]
        T = estimate_transition_factor_with_freq_and_energy_dimer(energy, frequency_list,scaling_factor,equal_energy_assumption= False)

        T_prime_prefactor = estimate_T_prime_prefactor_subroutine_dimer(alpha_list, frequency_list, energy, equal_energy_assumption = False)
        T_prime = Vt * T_prime_prefactor

        T_list_energy_in_one_monomer.append(T)
        T_prime_list_energy_in_one_monomer.append(T_prime)

    T_T_prime_sum_estimate = np.array(T_prime_list_energy_in_one_monomer) + np.array(T_list_energy_in_one_monomer)
    ...
    # T
    ax.scatter(state_energy, T_states , marker='o', color='blue', s=40)
    ax.plot(energy_list, T_list_energy_in_one_monomer, color='red', linewidth=2)

    ax.set_xlabel('$E$')
    ax.set_ylabel("transition factor")
    ax.set_xticks([5000, 10000, 15000, 20000])

    # estimate  transition energy
    T_transition_energy_index_rank = np.argsort(np.abs(np.array(T_list_energy_in_one_monomer )-1) )
    T_transition_energy = np.mean( energy_list[T_transition_energy_index_rank[:2]])
